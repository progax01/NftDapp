...
function Upload() {
  ...
  const [mintingInProgress, setMintingInProgress] = useState(false); // New state
  ...
  const uploadMetadata = async () => {
    try {
      ...
      toast.success(" IPFS uploaded successfully!");
  ...
  const mintNFT = async (tokenURI, price) => {
    ...
    if (accounts.length === 0) {
      console.error(
        "No Ethereum accounts available. Please unlock your wallet."
      );
      return;
    }
    ...
    try {
      ...
      web3.eth
        ...
        .on("transactionHash", (hash) => {
          ...
          toast.success(`NFT Minted Successfully! ${hash}`);
    ...
    <Layout>
      <div className="w-full max-w-md mx-auto">
        <h2 className="text-xl font-semibold mb-4">
          Upload Image and Mint Your NFT
        </h2>
      ...
      {mintingInProgress && (
          <div className="text-center mt-4">
            <div className="bg-yellow-400 text-white font-bold py-2 px-4 rounded">
              Minting in Process...
            </div>
          </div>
        )}
